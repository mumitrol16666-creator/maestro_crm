...
const { selectPreviousGroupHomework } = require('./groupHomeworkSource');
...
function mapStudentLesson(cls, now = new Date(), homeworkReview = null, reviewConfidence = null) {
    ...
    return {
        ...
        reviewConfidence: published ? reviewConfidence : null,
                ...
                return {
                    ...
                    memberships: teacherMemberships.map((membership) => {
                        ...
                        return {
                            ...
                            lessonRates: membership.lessonRates,
                            type: membership.type,
                            planName: membership.tariffName || membership.plan?.name || null,
                            lessonFormat: membership.lessonFormat,
                            lessonPrice: estimate.lessonPrice,
                            classesRemaining: membership.billingModel === 'rate_card' ? null : estimate.estimatedLessonsRemaining ?? membership.classesRemaining,
                            endDate: membership.billingModel === 'rate_card' ? null : membership.endDate,
...
async function getClassStudents(crmClassId) {
    ...
    let roster = [];
    let previousGroupHomework = null;
    ...
    if (cls.individualStudent) {
        ...
        const previousGroupClasses = await prisma.class.findMany({
            where: {
                groupId: cls.groupId,
                id: { not: cls.id },
                status: 'completed',
                OR: [
                    { date: { lt: cls.date } },
                    { date: cls.date, startTime: { lt: cls.startTime } },
                ],
            },
            select: {
                id: true,
        ...
        previousGroupHomework = selectPreviousGroupHomework(previousGroupClasses);
    }
                ...
                class: {
                    ...
                    ...(cls.groupId
                        ...
                        : {
                            ...
                            individualStudentId: { in: realStudentIds },
                        }),
                },
            },
            select: {
...
async function getStudentOfflineSummary(crmStudentId) {
        ...
        select: {
            ...
            memberships: {
                ...
                select: {
                    ...
                    programMonths: true, individualBudgetTotal: true, individualBudgetRemaining: true,
                    additionalDiscountType: true, additionalDiscountBasisPoints: true, additionalDiscountAmount: true, additionalDiscountReason: true,
    ...
    const mapMembership = (m) => ({
        ...
        lessonRates: m.lessonRates,
        type: m.type,
        planName: m.tariffName || (m.lessonFormat === 'program' ? 'Основная программа' : m.plan?.name || null),
        ...
        programMonths: m.programMonths,
        individualLessonPrice: m.individualLessonPrice,
        groupLessonPrice: m.groupLessonPrice,
        theoryLessonPrice: m.theoryLessonPrice,
        individualBudgetTotal: m.individualBudgetTotal,
        individualBudgetRemaining: m.individualBudgetRemaining,
        additionalDiscountAmount: m.additionalDiscountAmount,
        classesRemaining: m.billingModel === 'rate_card' ? null : m.classesRemaining,
        ...
        totalClasses: m.billingModel === 'rate_card' ? null : m.totalClasses,
        startDate: m.billingModel === 'rate_card' ? null : m.startDate,
        endDate: m.billingModel === 'rate_card' ? null : m.endDate,
