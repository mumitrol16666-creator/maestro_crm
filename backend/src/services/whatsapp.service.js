    ...
    async processBufferedMessages(phoneNumber, settings) {
        ...
        try {
            ...
            if (extractedData) {
                ...
                // Сохраняем реальный номер телефона (если клиент его назвал)
                if (extractedData.realPhone && !conversation.realPhone) {
                    conversation.realPhone = extractedData.realPhone;
                    await conversation.save();
                    console.log(`📱 [WhatsApp] Сохранён номер телефона: ${extractedData.realPhone}`);
                }
            }
            ...
            if (shouldCreateBooking) {
                try {
                    ...
                    console.log(`📝 [WhatsApp] Создана заявка #${booking._id} для ${phoneNumber}${conversation.isLead ? ' (Lead ID)' : ''}`);
