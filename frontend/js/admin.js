...
async function updatePendingAttendanceBadge() {
    try {
        ...
        if (badge) {
            ...
            console.warn('⚠️ Элемент pendingAttendanceBadge не найден');
        }
    } catch (error) {
...
function getMembershipClass(membership) {
    if (!membership || membership.status === 'expired' || membership.classesRemaining === 0) {
        return 'expired';
    }
    if (membership.classesRemaining === 1) {
        return 'critical'; // Красный для 1 занятия
    }
    if (membership.classesRemaining === 2) {
...
// ========================================
// УПРАВЛЕНИЕ ПРАВАМИ РОЛЕЙ - ИНТЕРАКТИВНАЯ СИСТЕМА
// ========================================
