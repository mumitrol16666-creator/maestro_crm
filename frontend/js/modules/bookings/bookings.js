...
function getWhatsappLink(phone) {
    ...
    const safeRaw = escapeBookingText(raw || '—');
    if (!digits) {
        ...
        convertForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const confirmed = typeof customConfirm === 'function'
                    ? await customConfirm(
                        'Создать карточку ученика? Заявка исчезнет из рабочей очереди и останется в аналитике.',
                        { icon: 'warning', yesText: 'Создать ученика', noText: 'Отмена' }
                    )
                    : window.confirm('Создать карточку ученика? Заявка исчезнет из рабочей очереди и останется в аналитике.');
                if (!confirmed) return;
                ...
                if (convertData.success) {
                    ...
                    setTimeout(() => {
                        ...
                        // После создания ученика заявка исчезает из рабочей очереди.
                        renderBookings(currentBookingFilter, currentBookingSearch, currentBookingPage);
